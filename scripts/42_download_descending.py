#!/usr/bin/env python
"""
Phase II-A - retrieve the DESCENDING HyP3 multi-burst products.

Independent validation product. Does not read or write the ascending product.

Retrieval discipline is inherited from the production run, where a transient ASF
DNS outage killed the first attempt at 47/336:

  * `connect_hyp3` and job listing are retried with backoff;
  * the batch is listed with ONE call, never `hyp3.refresh(Batch)` (which issues
    one HTTP request per job);
  * terminal state is read from `status_code`, because `Job.succeeded` /
    `Job.failed` are METHODS and are always truthy as attributes;
  * the append-only product inventory records every download, including failures.

Usage
-----
    python scripts/42_download_descending.py [--wait] [--workers N]
"""
...
FILE_INVENTORY_PATH = MANIFEST_DIR / "descending_file_inventory.csv"
...
def main() -> int:
    ...
    parser.add_argument("--workers", type=int, default=8,
                        help="parallel product downloads (I/O bound)")
    ...
    print("=" * 88)
    print("PHASE II-A - DESCENDING RETRIEVAL")
    print("=" * 88)
    ...
    while True:
        ...
        pending = [j for j in succeeded if str(j.job_id) not in done]
        # Downloads are I/O bound, so several can run at once. The inventory
        # writers are NOT thread-safe (they read-modify-write a CSV), so results
        # are collected first and appended serially afterwards.
        if args.workers > 1 and len(pending) > 1:
            with ThreadPoolExecutor(max_workers=args.workers) as pool:
                fetched = list(pool.map(download_and_extract, pending))
        else:
            fetched = [download_and_extract(job) for job in pending]
        for job, (record, file_rows) in zip(pending, fetched):
