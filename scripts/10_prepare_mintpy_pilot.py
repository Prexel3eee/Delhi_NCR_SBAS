...
common_overlap.json       overlap geometry and clipping provenance
ingestion_report.json     what MintPy actually loaded
...
def select_products(inventory: pd.DataFrame) -> pd.DataFrame:
    """One copy per unique water-mask=ON configuration."""
    ok = inventory[inventory["download_ok"].fillna(False).astype(bool)]
    masked = ok[ok["apply_water_mask"] == True]  # noqa: E712
    return (
        masked.sort_values("extracted_bytes", ascending=False)
...
def write_template(records: list[dict], provenance: dict) -> Path:
    ...
    lines = [
        ...
        "# Generated by scripts/10_prepare_mintpy_pilot.py",
    ...
    path = MINTpy_DIR / "mintpy_pilot.txt"
    path.write_text("\n".join(lines))
    return path
...
def main() -> int:
    ...
    (MINTpy_DIR / "common_overlap.json").write_text(json.dumps(provenance, indent=2))
    (MINTpy_DIR / "ingestion_report.json").write_text(json.dumps(report, indent=2, default=str))
    ...
    print(f"  report                : {MINTpy_DIR / 'ingestion_report.json'}")
