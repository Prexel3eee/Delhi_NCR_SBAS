...
def verify_coverage() -> tuple[bool, dict]:
    """Every acquisition date must have an ERA5 profile."""
        ...
        with h5py.File(era5_h5, "r") as handle:
            ...
            for key in handle.keys():
                if hasattr(handle[key], "shape"):
                    h5_info[f"{key}_shape"] = list(handle[key].shape)
                    if handle[key].shape and handle[key].shape[0] in (len(dates), len(grbs)):
                        vals = np.array(handle[key])
                        h5_info[f"{key}_finite_fraction"] = round(
                            float(np.isfinite(vals).mean()), 6
                        )
                        break

    complete = (not missing) and (not too_small) and era5_h5.exists()
    report = {
...
def main() -> int:
    ...
    residual = per_ifg_residuals(ERA5, window, mask)
